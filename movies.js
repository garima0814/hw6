...
// https://api.themoviedb.org/3/movie/now_playing?api_key=${030138f66d4cf0fb6dc745580f67503a}&language=en-US
...
window.addEventListener('DOMContentLoaded', async function(event) {
    ...
    let url = 'https://api.themoviedb.org/3/movie/now_playing?api_key=030138f66d4cf0fb6dc745580f67503a&language=en-US'
    ...
    let moviesId = movies[i].id
    let posterUrl = movies[i].poster_path

    document.querySelector('.movies').insertAdjacentHTML('beforeend', `
    <div class="w-1/5 p-4 movies-${moviesId}">
    ...
    console.log(`${moviesId} was watched!`)

    await db.collection('watched').doc(`${moviesId}`).set({
    })

    let docRef = await db.collection('watched').doc(`${moviesId}`).get()
